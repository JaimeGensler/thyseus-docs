...
Resources are modeled as classes. Unlike components, they _can_ be, but are not
...
// This resource can be accessed from any thread
@struct
class Time {
	previous: f64 = 0;
	current: f64 = 0;
...
// This resource can only be accessed from the main thread
class Renderer {
	draw() {
		// ...
	}
