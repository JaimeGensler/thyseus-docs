...
-   `createDefault(): void` adds an element to the queue with the default data
    for that struct.
-   `create(): T` adds an element to the queue and returns a handle to that
    element for you to mutate.
-   `createFrom(instance: T): void` copies the data from passed instance of the
...
`EventWriter` also inherits all the functionality of `EventReader`!

## Parallelism

`EventWriter`s require exclusive access to their queue, and so intersect with
readers and writers of the same type. `EventReader`s do _not_ require exclusive
