...
<Opinion type="negative" title="Bad">
    ```ts
    const myWorld = await World
		.new({ threads: 2 }, './aDifferentModule');
    ```
</Opinion>

<Opinion type="negative" title="Also bad">
	```ts
	const myWorldBuilder = await World
		.new({ threads: 2 }, import.meta.url)
...
<Opinion type="positive" title="Okay, but not great">
	```ts
	const world = await World.new().build();
