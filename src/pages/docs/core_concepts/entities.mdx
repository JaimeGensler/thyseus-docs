...
to access the entity's id, and can be queried for in systems (_queries will be
covered more later, too!_).
...
```ts
function destroyEntity(commands: Commands) {
	const entity = commands.spawn();

	// If you have the `EntityCommands` object
	entity.despawn();
